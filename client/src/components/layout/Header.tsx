...
export default function Header() {
  ...
  const getUserInfo = () => {
    ...
    return {
      ...
      subscriptionPlan: 'basic' // Default for now
    };
  };
        ...
        <div className="max-w-full mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          <div className="flex justify-between items-center h-14 sm:h-16 md:h-16">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 lg:space-x-8">
              <Link href="/" onClick={scrollToTop} className="flex items-center space-x-2 sm:space-x-3 cursor-pointer transition-all duration-200 group">
                <img
                  src="/logo-gold.png"
                  alt="Sharp Shot Logo"
                  className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 flex-shrink-0 group-hover:scale-110 transition-transform duration-200"
                />
                <span className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-extrabold text-gray-900 dark:text-white group-hover:text-[#D8AC35] dark:group-hover:text-[#D8AC35] group-hover:scale-110 flex-shrink-0 transition-all duration-200" style={{ fontFamily: "'Saira Condensed', sans-serif", fontStyle: 'italic', transform: 'skew(-5deg)', fontWeight: 'inherit' }} onMouseEnter={(e) => e.currentTarget.style.fontWeight = '900'} onMouseLeave={(e) => e.currentTarget.style.fontWeight = '800'}>Sharp Shot</span>
              </Link>
              ...
              <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
                <Link href="/trading-terminal">
                  <button
                    onClick={scrollToTop}
                    className={`text-gray-700 dark:text-gray-300 hover:text-[#D8AC35] dark:hover:text-[#D8AC35] hover:scale-110 font-extrabold transition-all duration-200 py-2 px-2 lg:px-3 h-10 lg:h-12 cursor-pointer text-sm lg:text-base ${
                    location === '/trading-terminal' ? 'text-[#D8AC35]' : ''
                  }`}>
                    Trading Terminal
                  </button>
                </Link>
                {FEATURE_FLAGS.NEW_TERMINAL && (
                  <Link href="/trading-terminal-new">
                    <button
                      onClick={scrollToTop}
                      className={`text-gray-700 dark:text-gray-300 hover:text-[#D8AC35] dark:hover:text-[#D8AC35] hover:scale-110 font-extrabold transition-all duration-200 py-2 px-2 lg:px-3 h-10 lg:h-12 cursor-pointer text-sm lg:text-base ${
                      location === '/trading-terminal-new' ? 'text-[#D8AC35]' : ''
                    }`}>
                      Try New Terminal
                    </button>
                  </Link>
                ...
                <div>
                  <button
                    ...
                    className="text-gray-400 dark:text-gray-600 font-extrabold py-2 px-2 lg:px-3 h-10 lg:h-12 cursor-not-allowed opacity-60 text-sm lg:text-base"
                    disabled>
            ...
            <div className="flex items-center space-x-6">
              ...
              <div className="hidden md:flex items-center space-x-6">
                ...
                <div className="relative group">
                  <button
                    className="relative text-gray-700 dark:text-gray-300 hover:text-[#D8AC35] dark:hover:text-[#D8AC35] hover:scale-110 font-bold transition-all duration-200 py-2 px-2 lg:px-3 h-10 lg:h-12 flex items-center focus:outline-none cursor-pointer text-sm lg:text-base"
                    aria-haspopup="true"
                ...
                <Link
                  href="/pricing"
                  onClick={scrollToTop}
                  className={`text-gray-700 dark:text-gray-300 hover:text-[#D8AC35] dark:hover:text-[#D8AC35] hover:scale-110 font-bold transition-all duration-200 py-2 px-2 lg:px-3 h-10 lg:h-12 flex items-center cursor-pointer text-sm lg:text-base ${
                  ...
                  <>
                    {userInfo?.subscriptionStatus !== 'active' && (
                      <Link
                        href="/subscribe"
                        onClick={scrollToTop}
                        className="text-[#D8AC35] hover:text-black dark:hover:text-white hover:scale-110 font-extrabold transition-all duration-200 py-2 px-2 lg:px-3 h-10 lg:h-12 flex items-center cursor-pointer text-sm lg:text-base"
                      >
                        Get Started Free
                      </Link>
                    )}
                    ...
                    <DropdownMenu>
                      <DropdownMenuTrigger className="flex items-center space-x-2 focus:outline-none">
                        <Avatar className="h-8 w-8">
                          <AvatarImage src={userInfo?.profilePhoto || ""} alt={userInfo?.username || user.email} />
                          <AvatarFallback className="bg-[#D8AC35] text-black text-sm font-semibold">
                            {(userInfo?.username || user.email || "U").charAt(0).toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-56">
                        <div className="flex items-center space-x-2 p-2">
                          ...
                          <div className="flex flex-col space-y-1">
                            <p className="text-sm font-medium">{userInfo?.username || user.email}</p>
              ...
              <div className="md:hidden flex items-center space-x-2">
                {!user && (
                  <Link href="/register">
                    <Button
                      onClick={scrollToTop}
                      className="bg-[#D8AC35] text-gray-900 px-2 sm:px-3 py-1.5 rounded-md shadow-sm hover:bg-[#c49429] transition-all duration-200 font-semibold text-xs sm:text-sm">
                ...
                <Sheet open={isOpen} onOpenChange={setIsOpen}>
                  <SheetTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-10 w-10 sm:h-11 sm:w-11 rounded-md">
                  ...
                  <SheetContent side="right" className="w-[300px] sm:w-[350px]">
                    <div className="flex flex-col space-y-6 mt-8">
                      ...
                      <div className="space-y-4">
                        ...
                        {FEATURE_FLAGS.NEW_TERMINAL && (
                          <Link
                            href="/trading-terminal-new"
                            className={`block py-3 px-4 rounded-lg transition-colors min-h-[44px] ${
                              location === '/trading-terminal-new'
                                ? 'bg-[#D8AC35] text-gray-900'
                                : 'text-gray-700 hover:bg-gray-100'
                            }`}
                            onClick={() => {
                          ...
                          >
                            <div className="font-semibold">Try New Terminal</div>
                            <div className="text-sm opacity-75 mt-1">Preview the enhanced terminal</div>
                          </Link>
                        )}
                        <Link
                          ...
                          <>
                            ...
                            {userInfo?.subscriptionStatus !== 'active' && (
                              <Link href="/subscribe" onClick={() => {
                                localStorage.setItem('subscriptionIntent', 'subscribe');
                                setIsOpen(false);
                              }}>
