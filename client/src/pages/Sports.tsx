  ...
  const { data: recentEvents, isLoading: eventsLoading } = useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const games = await sportsApi.getGames();
        const recentGames = games.slice(0, 20); // Limit to 20 most recent
        
        return {
          events: recentGames.map(game => ({
            id: game.gameID,
            sport: game.sport,
            homeTeam: game.team1Name,
            awayTeam: game.team2Name,
            startTime: new Date(game.time || game.date).toISOString(),
            status: game.timeLeft,
