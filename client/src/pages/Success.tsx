...
import { CheckCircle, Download, Users, TrendingUp, Loader2 } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

export default function Success() {
  const [user, setUser] = useState<any>(null);
  const [sessionStatus, setSessionStatus] = useState<'loading' | 'complete' | 'open' | 'error'>('loading');
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      ...
      setSessionStatus('complete'); // Fallback for direct access
    }
  }, []);
      ...
      if (response.ok) {
        ...
        if (data.status === 'complete') {
          ...
          toast({
            ...
            description: "Your payment is still processing. You'll be redirected to complete it.",
            variant: "destructive",
          });
          // Redirect back to subscribe page
