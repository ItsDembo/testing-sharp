...
const convertOpportunitiesToPresets = (opportunities: any[]) => {
  return opportunities.map((opp, index) => ({
    ...
    ev: opp.ev,
    winRate: opp.hit, // NO SYNTHETIC VARIATION
    followers: null, // NO SYNTHETIC FOLLOWERS
    volume: null, // NO SYNTHETIC VOLUME
    roi: opp.ev, // NO SYNTHETIC ROI VARIATION
...
export default function Views() {
  ...
  const { data: opportunitiesData, isLoading: isLoadingOpportunities } = useQuery({
    queryKey: ['live-opportunities', { sport: activeSport, minEV: minEV }],
    queryFn: async () => {
      try {
        // Get live games from the API
        const liveGames = await sportsApi.getLiveGames();
        
        // Filter by sport if specified
        let filteredGames = liveGames;
        if (activeSport !== "all") {
          filteredGames = liveGames.filter(game => game.sport === activeSport);
        }
        
        // Convert games to opportunities format
        const opportunities = filteredGames.map(game => ({
          id: game.gameID,
          sport: game.sport,
          game: `${game.team2Name} @ ${game.team1Name}`,
          market: 'Game',
          betType: 'Moneyline',
          line: game.timeLeft,
          ev: null, // NO SYNTHETIC EV
          confidence: 'Medium',
  ...
  const { data: terminalStats } = useQuery({
    ...
    queryFn: async () => {
      try {
        // Get live games count and upcoming games count
        const [liveGames, upcomingGames] = await Promise.all([
          sportsApi.getLiveGames(),
          sportsApi.getUpcomingGames()
        ]);
        
        return {
          liveGamesCount: liveGames.length,
          upcomingGamesCount: upcomingGames.length,
          totalOpportunities: liveGames.length + upcomingGames.length,
          lastUpdate: new Date().toISOString()
        };
      } catch (error) {
        console.error('Failed to fetch terminal stats:', error);
        return {
          liveGamesCount: 0,
