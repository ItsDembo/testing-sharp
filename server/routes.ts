...
import liveGameValidationRoutes from './routes/liveGameValidation';
import tradingTerminalRoutes from './routes/tradingTerminal';
...
import { generateNonce, verifySignature } from './routes/siws';
import { handleGoogleAuth, getGoogleAuthUrl } from './routes/google-auth';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // SIWS (Sign In With Solana) Routes
  ...
  app.use('/api/live-games', liveGameValidationRoutes);
  ...
  app.get("/api/trading-terminal/data", async (req, res) => {
    const { getTradingTerminalData } = await import('./routes/trading-terminal');
    return getTradingTerminalData(req, res);
  });
  
  app.get("/api/trading-terminal/stats", async (req, res) => {
    const { getTradingTerminalStats } = await import('./routes/trading-terminal');
    return getTradingTerminalStats(req, res);
  });
