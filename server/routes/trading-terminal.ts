...
export const getTradingTerminalData = async (req: Request, res: Response) => {
  try {
...
export const getTradingTerminalStats = async (req: Request, res: Response) => {
  try {
    const { sport } = req.query;
    ...
    const stats = {
      ...
      leagues: [...new Set(events.map(e => e.league))],
      markets: [...new Set(events.map(e => e.market))],
