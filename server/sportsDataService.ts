...
export class SportsDataService {
  private async makeApiCall(endpoint: string, params: Record<string, string | number> = {}): Promise<any> {
    ...
    // Add API key to all requests using 'apiKey' parameter (as per API documentation)
    ...
    if (contentType.includes('application/json')) {
      ...
      // For XML responses, return the text for parsing elsewhere
      const text = await response.text();
      console.log(`API XML response for ${endpoint}: ${text.substring(0, 200)}...`);
      return text;
    }
  }
  ...
  // Get future headlines for upcoming games using the headlines API endpoint
  ...
  // Get past headline games using the headlines API endpoint
